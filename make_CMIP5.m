...
%  make_CMIP5.m
%
%  Create and fill frc and bulk files with CMIP5 data.
%
%  Further Information:
...
%
% Get the CMIP5 horizontal grids (it should be the same for every month)
%
nc=netcdf([CMIP5_dir,'LSM_Y',num2str(Ymin),'M',num2str(Mmin),'.nc']);
...
for Y=Ymin:Ymax
    ...
    for M=mo_min:mo_max
        ...
        %
        %-------------------------------------------------------------------%
        nc=netcdf([CMIP5_dir,'T2M_Y',num2str(Y),'M',num2str(M),'.nc']);
        CMIP5_time=nc{'time'}(:);
        close(nc);
        dt=mean(gradient(CMIP5_time));
        disp(['dt=',num2str(dt)])
        %-----------------------------------------------------------
        %Variable overlapping timesteps : 2 at the beginning and 2 at the end
        %------------------------------------------------------------
        tlen0=length(CMIP5_time);
        disp(['tlen0=',num2str(tlen0)])
        freq=1; % hourly
        itolap=freq*itolap_cmip5;
        tlen=tlen0+2*itolap;
        disp(['tlen=',num2str(tlen)])
        disp(['Overlap is ',num2str(itolap_cmip5),' records before and after'])
        time=0*(1:tlen);
        time(itolap+1:tlen0+itolap)=CMIP5_time;
        disp(['====================='])
        ...
        %
        % Check if there are CMIP5 files for the previous Month
        Mm=M-1;
        Ym=Y;
        if Mm==0
            Mm=12;
        ...
        %
        fname = [CMIP5_dir,'TP_Y',num2str(Ym),'M',num2str(Mm),'.nc'];
        %nc=netcdf([CMIP5_dir,'TP_Y',num2str(Ym),'M',num2str(Mm),'.nc']);
        %
        disp(' ')
        disp('======================================================')
        ...
        for aa=1:itolap
            ...
            interp_CMIP5(CMIP5_dir,Ym,Mm,Roa,interp_method,lon1,lat1,...
        ...
        for tndx=1:tlen0
            ...
            interp_CMIP5(CMIP5_dir,Y,M,Roa,interp_method,lon1,lat1,...
        ...
        disp(' ')
        %######################################################################
        % Read CMIP5 file for the next month
        %
        Mp=M+1;
        Yp=Y;
        if Mp==13
        ...
        fname=[CMIP5_dir,'TP_Y',num2str(Yp),'M',num2str(Mp),'.nc'];
        ...
        for tndx=tlen0+itolap+1:tlen;
            ...
            interp_CMIP5(CMIP5_dir,Yp,Mp,Roa,interp_method,lon1,lat1,...
